...
\item Fails to provide payment to the broker with a given probability
\item Fails to provide correct payment amount to the broker with a given probability
\item Fails to provide payment transfer for the correct node to the broker with a given probability
\item Permanently stops consuming data with a given probability
\end{itemize}
...
\begin{itemize}
\item Fully cooperative
\item Fails to provide correct data with a given probability
\item Provides data at a slow rate with a given probability
\item Permanently stops providing data with a given probability
\end{itemize}
...
\begin{itemize}
\item Fully cooperative
\item Fails to store currency transfer authorizations with a given probability
\item Accepts invalid transfer authorizations with a given probability
\item Declines valid transfer authorizations with a given probability
\item Fails to forward transfered currency units to the proper bank nodes with a given probability
\item Permanently stops providing bank node functions with a given probability
\end{itemize}
...
\begin{itemize}
\item Fully cooperative
\item Sends decryption key to consumer without proper payment with a given probability
\item Sends decryption key to consumer without forwarding payment transfer to bank nodes with a given probability
\item Sends invalid decryption key to consumer after authorizing payment
\item Fails to validate that payment transfer is for correct provider with a given probability
\end{itemize}
