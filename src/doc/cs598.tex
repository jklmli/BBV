...
\title[Reputation Management with Distributed Brokers]{%
       Reputation Management in P2P Storage Using Virtual Currency Exchange and Distributed Brokers}
...
\PARstart The full potential peer to peer systems have to change the way computing is done has yet to be fully realized.  Users should be wary of trusting individual corporations with the privacy and security of their data.  When appropriate encryption is used, a peer to peer cloud has the potential to be more private and secure.  With peer to peer systems corporations can utilize some of the idle resources of their users to provide services whose capacity scales naturally with the number of users at significantly reduced cost.  Peer to peer can also level the playing field giving individuals and small companies the opportunity to try new ideas and compete with large corporations with vast resources.  
...
\end{itemize}
The state of the network is expected to be constantly changing.  Data consumers should be capable of automatically adjusting their valuations as network conditions change. ~\cite{xiong} ~\cite{milano} ~\cite{kamvar} ~\cite{oualha}
